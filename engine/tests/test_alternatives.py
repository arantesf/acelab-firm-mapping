"""Every mapped decision offers ranked, grounded alternatives (top-3, best-first).

The engine surfaces up to three products per element so a human can choose, but the
guarantees are the same as for the single pick: every option is in the catalog, qualifies
for the matched standard, carries its own composite confidence, and has re-derived write
parameters. `alternatives[0]` is the engine's own primary and mirrors `chosen_product`.
"""
...
def test_every_mapped_decision_has_grounded_alternatives(engine, decisions):
    ...
    for d in decisions:
        ...
        for alt in d.alternatives:
            ...
            assert alt.confidence is not None
...
def test_bogus_alternative_ids_are_dropped_and_list_is_padded():
    ...
    engine = Engine(catalog, standards, _Stub(raw), sync_date="2026-07-12")
